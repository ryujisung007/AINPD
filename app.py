...
맨 위에 "질문을 받으면 이 문서를 먼저 보고 어느 파일을 볼지 정하세요"라고 적어줘.""",
        ...
        st.code("""- 대화창에 올린 파일이 있으면 그 파일을 먼저 분석한다.
  지식에 넣어둔 자료는 내가 시킬 때만 본다.
- 배합비, 원료, 사용량, 규격에 관한 질문은 자료를 근거로 답한다.
- 수치를 말할 때는 어느 파일에서 봤는지 파일명을 함께 적는다.
- 자료에 없으면 "자료에 없습니다"라고 먼저 말하고, 내 일반 지식이면 그렇다고 밝힌다.
- 파일이 안 읽히면 "이 파일을 읽지 못했습니다"라고 파일명과 함께 말한다.
- 내가 "자료 기준으로"라고 하면 어떤 질문이든 자료만 근거로 답한다.
- 그 밖의 질문은 평소처럼 답한다.""", language=None)

        st.caption(
            "💡 배합비·원료 질문에만 자료를 뒤지고, 일반 질문은 평소처럼 답합니다. "
            "자료를 꼭 근거로 삼게 하려면 질문 앞에 **\"자료 기준으로\"** 를 붙이세요."
        )
