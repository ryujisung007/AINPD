...
_SUBMIT_INSTRUCTION = (
    ...
    "- 에세이 문체(서술형)로 작성할 것\n"
    "- 글자 수: 정확히 200자 이상 230자 이하 (공백 포함, 반드시 충족)\n"
    "[제출답안]: 여기에 에세이 문체로 200~230자 핵심 요약 작성"
)
...
try:
    ...
    _ADMIN_CODE = "kfi2026admin"  # 로컬 실행 기본값
...
[이 대화 전용 역할 지정] 지금부터 음료연구원은 이 페르소나를 적용
...
[이 대화 전용 역할 지정] 지금부터 음료마케터는 이 페르소나를 적용
[페르소나 출력 후 간단한 질문으로 적용확인]
...
[이 대화 전용 역할 지정] 지금부터 이 연구원은 위 페르소나를 적용
[적용 확인] {r_test_q}"""
...
[이 대화 전용 역할 지정] 지금부터 이 마케터는 위 페르소나를 적용
[적용 확인] {m_test_q}"""
...
데이터 출처가 있으면 함께 명시해줘. 가짜 데이터나 추정값은 '추정' 표시 필수.""", language=None)
        ...
        with sub_task:
            ...
            _INDIRECT_PLATFORMS = {"쿠팡", "마켓컬리", "편의점 앱(CU·GS)"}
            ...
            with col_hint1:
                ...
                if hint_platform in _INDIRECT_PLATFORMS:
                    st.caption("⚠️ 이 플랫폼은 내부 판매 데이터가 비공개입니다 — 언론·리뷰·SNS 등 간접 정보로 추정하는 스크립트가 자동 반영됩니다.")
...
<span style="font-size:12px;color:#92400e;margin-left:8px;">스크립트에 추가할 내용을 자유롭게 입력하세요 (초안에 자동 포함)</span>
            ...
            if st.button("✏️ 선택 항목으로 초안 생성", key="hint_gen_btn", use_container_width=True):
                ...
                if _plat in _INDIRECT_PLATFORMS:
                    _basis = "언론 보도·리뷰 블로그·SNS 언급·브랜드 공식 발표 등 공개된 간접 정보 (최근 6개월)"
                    _note = ("※ 이 플랫폼은 내부 판매 데이터가 비공개이므로 직접 크롤링하지 말고, "
                              "공개된 간접 정보를 근거로 추정해줘. 모든 수치는 추정치이며 항목별로 "
                              "근거 출처(기사·리뷰·SNS 등)를 반드시 함께 표시해줘.")
                else:
                    _basis = "최신 판매량 + 리뷰수 + 별점 기준 상위 제품 (최근 6개월)"
                    _note = "※ 가짜 데이터 금지. 추정값은 '추정' 표시 필수."
...
{_note}"""
            ...
            if user_script.strip():
                st.caption("📋 아래 코드 블록 우측 상단 복사 아이콘 클릭 → ChatGPT, Gemini 등에 붙여넣기")
                st.code(user_script, language=None)
            ...
            if hint_platform in _INDIRECT_PLATFORMS:
                _CHECKLIST.append("비공개 플랫폼 특성상 간접 추정 근거·출처 표시를 요청했나요?")
            checks = []
            ...
            passed = sum(checks)
            if passed == total:
                st.success(f"✅ 완벽합니다! {total}개 항목을 모두 충족하는 좋은 스크립트입니다.")
            elif passed >= 3:
                st.warning(f"🔶 {passed}/{total} 충족 — 미체크 항목을 스크립트에 추가하면 더 좋은 결과를 얻을 수 있어요.")
            elif user_script.strip():
                st.error(f"⚠️ {passed}/{total} 충족 — 예시 스크립트를 다시 참고해 부족한 항목을 보완하세요.")
            else:
    ...
    with tab_food:
        ...
        with sub_food_ex:
            ...
            st.warning("⚠️ 데이터 분석용 토큰 소모가 크므로, 분석용 파일은 **혼합음료 혹은 과채주스 품목제조보고서 파일만** 업로드해주세요.")
...
{_fothers_str}"""
            ...
            if food_script.strip():
                st.caption("📋 아래 코드 블록 우측 상단 복사 아이콘 클릭 → ChatGPT, Gemini 등에 붙여넣기")
                st.code(food_script, language=None)
    ...
    with tab_learn:
        ...
        with sub_learn_ex:
            ...
            st.warning("⚠️ 데이터 분석용 토큰 소모가 크므로, 다운로드한 5장 중 **선택한 2장만** 업로드해주세요.")
...
5. 보고서를 아래의 양식으로 작성
   {_lrep_str}"""
            ...
            if learn_script.strip():
                st.caption("📋 아래 코드 블록 우측 상단 복사 아이콘 클릭 → ChatGPT, Gemini 등에 붙여넣기")
                st.code(learn_script, language=None)
...
→ AI가 업로드된 소스 파일을 참조해 답변합니다.""")
        ...
        st.markdown("---")
        st.markdown("##### 프로젝트 소스파일 활용방법")
        st.markdown("""
| 파일명 | 내용 | 활용 시점 |
|---|---|---|
| 음료시장 세분화데이터 | 시장정보 | 아이디어도출단계 |
| 음료제조기술과이론 | 배합비이론 | 배합비설계단계 |
| 신제품관능평가방법 | 맛엔진 학습데이터 | 소비자조사단계 |
| 식품공전 | 식품유형별 법규 | 출시전 인허가단계 |
| 음료개발데이터베이스 | 배합비 작성법 학습엔진 | 배합비 실무활용단계 |
""")
...
# ----------------------------------------------------------
    ...
    tab_db, tab_ex, tab_write, tab_mission, tab_process = st.tabs([
        "📊 데이터베이스 구조",
        "📖 예시 스크립트",
        "✏️ 배합비 작성",
        "🎯 미션수행",
...
""")
        st.caption("💡 '예시 스크립트' 탭의 #사용자료 항목이 바로 이 데이터베이스를 가리킵니다.")
...
#출력결과
텍스트 표 형식으로 출력 (구글시트·엑셀 파일 생성 금지, 요약문으로 대체 금지)
...
누락 또는 오류 항목이 있으면 수정 후 전체 시트 다시 출력해줘.""", language=None)
        ...
        st.caption("AI가 출력한 시트에서 아래 값을 확인하여 입력하세요. 💡 입력 후 **Enter**를 눌러 적용하세요.")
...
- 100ml 원가: 기준량 환산 오류 → 전체 배합량(예: 3,000ml) 기준으로 재계산""",
        ...
        st.caption("💾 미션 2로 넘어가기 전에 미션 1 입력 내용을 먼저 제출해두세요. (미션 2까지 마친 뒤 최종 제출하면 아래 제출 내용이 최신 내용으로 갱신됩니다)")
