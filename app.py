...
def _hw_ui(sheet_tab: str, content: str, btn_key: str,
    ...
    if show_ai_field:
        st.markdown(
            ...
            '<span style="font-size:16px;color:#166534;font-weight:800;">📎 AI 생성결과</span>'
...
Step 2 &nbsp;·&nbsp; OpenAI에서 작성한 스크립트를 NotebookLM용 슬라이드 스크립트로 변환하세요
        ...
        st.code(
            "출력한 보고서를 노트북 lm용 보고서 작성 스크립트로 변환해주세요.\n"
            "노트북LM용 스크립트는 900자 이내로 작성하고, 파일로는 전부 내용을 담아서 출력해줘.",
            language=None,
        )
...
NotebookLM 슬라이드로 완성하여 발표 자료로 제출""", language=None)
        ...
        if not _s2_student:
            ...
            st.caption("OpenAI에서 변환한 NotebookLM 슬라이드 스크립트를 아래에 붙여넣고 제출하세요.")
            _nb_input = st.text_area(
                "NotebookLM 슬라이드 스크립트",
                key="report_nb_script",
                height=200,
                placeholder="OpenAI에서 변환된 NotebookLM 슬라이드 스크립트를 여기에 붙여넣으세요.",
                label_visibility="collapsed",
            )
