...
맨 위에 "질문을 받으면 이 문서를 먼저 보고 어느 파일을 볼지 정하세요"라고 적어줘.""",
        ...
        st.code("""[기본]
- 지식과 소스는 내가 쓰라고 할 때만 본다. 평소에는 뒤지지 마라.
- 대화창에 올린 파일이 있으면 그 파일을 먼저 분석한다.
- 내가 "자료 기준으로"라고 하면 지식과 소스를 근거로 답한다.

[자료를 볼 때]
- 수치는 어느 파일에서 봤는지 파일명을 함께 적는다.
- 자료에서 확인한 것과 네가 추론한 것을 나눠서 적는다.
  [자료] 파일명 - 내용
  [추론] 내용
...
- 파일이 안 읽히면 "이 파일을 읽지 못했습니다"라고 파일명과 함께 말한다.""",
        ...
        st.caption(
            ...
            "**파일을 올리면** 그 파일을 먼저 봅니다. 등록해둔 지식·소스를 쓰게 하려면 "
            "질문 앞에 **\"자료 기준으로\"** 를 붙이세요 — 배합비 개발이 대표적인 경우입니다."
        )
        st.caption(
