...
def _hw_ui(sheet_tab: str, content: str, btn_key: str,
    ...
    if show_ai_field:
        st.markdown(
            ...
            '<span style="font-size:11px;color:#166534;font-weight:600;">📎 AI 적용결과 붙여넣기</span></div>',
            unsafe_allow_html=True,
        )
        ai_result = st.text_area(
            "AI 적용결과", key=f"{btn_key}_ai",
...
결과는 3줄 이내로 핵심만 요약해주세요."""
        ...
        _hw_ui("배합비_프로세스", _proc_content, "proc_hw_submit",
               with_file=True, show_ai_field=False)
...
# ----------------------------------------------------------
    ...
    with tab_sensory:
        ...
        _hw_ui("관능검사", _sen_content, "sen_hw_submit", show_ai_field=False)
