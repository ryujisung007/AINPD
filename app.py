...
맨 위에 "질문을 받으면 이 문서를 먼저 보고 어느 파일을 볼지 정하세요"라고 적어줘.""",
        ...
        st.code("""[배합비 개발]
- 배합 설계, 원료 선정, 사용량 산출은 지식과 소스를 우선 근거로 삼는다.
- 수치는 어느 파일에서 봤는지 파일명을 함께 적는다.

[파일을 올렸을 때]
- 대화창에 올린 파일을 먼저 분석한다. 내가 무엇을 원하는지 판단해 진행한다.
- 자료에서 확인한 것과 네가 추론한 것을 반드시 나눠서 적는다.
  [자료] 파일명 - 내용
  [추론] 내용
- 자료에 없는 것을 자료에 있는 것처럼 쓰지 마라.
- 파일이 안 읽히면 "이 파일을 읽지 못했습니다"라고 파일명과 함께 말한다.

[지식 사용]
- 지식과 소스를 근거로 삼을지 애매하면, 답하기 전에 한 번 물어본다.
- 이미 물어봤거나 내가 "자료 기준으로"라고 하면 묻지 말고 바로 진행한다.
- 그 밖의 일반 질문은 자료를 뒤지지 말고 평소처럼 답한다.""", language=None)

        st.caption(
            "💡 **배합비**는 등록해둔 자료를 우선 봅니다. **파일을 올리면** 그 파일이 먼저입니다. "
            "그 밖에는 자료를 쓸지 AI가 한 번 물어봅니다 — "
            "묻지 말고 바로 쓰게 하려면 질문 앞에 **\"자료 기준으로\"** 를 붙이세요."
        )
        st.caption(
            "🔍 답변에 **[자료]** 와 **[추론]** 이 나뉘어 나옵니다. "
            "[자료]에는 파일명이 붙으므로 실제 근거인지 바로 확인할 수 있습니다."
        )
        with st.expander("❓ 파일을 올리면 자동으로 갱신되게 할 수는 없나요?"):
            st.markdown(
                ...
                "자료를 새로 올렸다면 **STEP 5를 다시 한 번** 돌려 "
                "`00_자료안내.md` 를 새로 만들어 덮어쓰면 됩니다."
            )
